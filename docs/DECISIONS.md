...
## 2026-09-21: 軽量SVGアイコンとConcept Diagramを共通ビジュアル言語にする

**Decision:** Poké Analyticsでは文章・数値に加え、共通の軽量SVGアイコンとHTML/CSS/最小限のSVGによる図解で情報を視覚的にも理解できるようにする。小アイコンはstable IDを解決する型安全な`SiteIcon`、説明図は再利用可能なConcept Diagram部品を基本とし、大きな挿絵画像とは役割を分ける。

**Reason:** OS依存emojiや画面ごとの独自記号を避けながら、スマートフォンでも記事の識別と仕組みの理解を速め、今後ほかの機能へ同じデザイン言語を展開できるようにするため。

**Consequence:** 24×24、`currentColor`、統一strokeのインラインSVGを基本とし、タイトル文字列からアイコンを推測しない。記事・カテゴリは明示的なicon IDを持つ。図解は本文を置き換えず、captionとテキストラベルを残し、色だけに依存しない。アイコンのためだけにclient componentや外部ライブラリ、個別network requestを増やさない。
