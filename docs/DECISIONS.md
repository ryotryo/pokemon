...
## 2026-09-22: タイプ相性は共通計算ロジックを唯一の情報源にする

**Decision:** 18×18タイプ相性表、複合タイプ判定、パーティー相性、ダメージ計算は、すべて`lib/champions/type-matchup.ts`の同じ相性定義と計算関数を使う。表示用の別表や記事内の重複データは作らない。

**Reason:** 相性変更や修正時に画面ごとの不一致を起こさず、単タイプ324通りと複合タイプの掛け合わせを同じテストで保証するため。

**Consequence:** `/type-chart`の表は攻撃タイプを縦、防御タイプを横に置き、モバイルでは表のラッパーだけを横スクロールさせる。倍率は色だけに依存せず文字でも表示する。複合タイプの同一タイプ重複は単タイプとして扱い、特性・技・場による例外は基本相性へ混ぜない。タイプ画像は共通Champions asset componentから表示する。
...
## 2026-09-23: ダメージ特性は効果の段階ごとに処理する

**Decision:** ダメージ早見表の特性処理は、特性名ごとの最終倍率ではなく、実効技タイプ、威力、攻撃・防御能力、無効化、最終補正の各段階へ割り当てる。技種別の判定にはChampions `waza.json`の分類を生成済みmove metadataとして再利用する。

**Reason:** スキン系のように技タイプ変更がSTABとタイプ相性の両方へ影響する特性や、防御側の無効・軽減を、最終ダメージへの一律乗算では正確に表現できないため。技名の手書き一覧はデータ更新時に欠落しやすいため。

**Consequence:** タイプ変更はSTAB・相性判定より前に行う。パンチ・音・切る・弾・波動・かみつき・反動のtagは更新処理で生成する。全合法特性は実装済み・条件付き・直接補正なしのいずれかへ分類し、ダメージへ関係しそうな未分類説明が追加された場合はcoverage testを失敗させる。現在の入力で一意に決まらない効果は、誤った倍率を入れず条件と理由をUIへ表示する。

## 2026-09-23: 通常作業のProduction pushは完成時の1回にまとめる

**Decision:** Deployment Storage節約のため、通常の作業途中checkpointはローカルcommitに留め、全検証後にmainへ原則1回だけpushする。

**Reason:** mainへのcheckpoint pushごとにProduction deploymentが作られ、保存容量を消費するため。

**Consequence:** セッション終了など作業消失の現実的な危険がある場合を除き、途中pushしない。ローカルcommitは複数作成してよい。最終push後にmain同期とProductionを確認する。
