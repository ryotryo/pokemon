...
| ポケモン タイプ相性表 | `/type-chart` | 18×18の攻撃・防御相性表と、複合タイプの弱点・耐性判定を表示する |
...
- 汎用の小アイコンは型安全な`SiteIcon`（24×24、`currentColor`のインラインSVG）を使い、個別画像リクエストやOS依存emojiを避ける。
- ポケモン、mini、タイプ、持ち物などゲーム内の具体物は、利用可能なChampions Battle Data画像を共通resolver/component経由で表示する。画像はテキストを補助するものとし、名称や重要情報を画像だけに依存させない。
- 説明のための図は、本文より明確に理解しやすくなる場合だけ個別に検討する。装飾目的や情報を重複させるだけの図は追加せず、本文だけで意味が完結する状態を保つ。
- タイプ相性は`lib/champions/type-matchup.ts`を唯一の計算元とし、相性表・複合タイプ判定・パーティー相性・ダメージ計算で共有する。
