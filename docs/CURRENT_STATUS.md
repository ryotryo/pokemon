...
2026-09-23（ダメージ早見表の全合法特性監査後に更新）

## Current focus

ダメージ早見表で選択できる全215特性を監査し、技タイプ・威力・攻撃防御能力・無効・最終補正へ分けた特性計算へ更新しました。条件が一意に決まる効果は計算し、現在の入力で決まらない効果は理由を表示します。
...
- ダメージ早見表は攻撃側・防御側双方の合法特性を計算へ渡す。スキン系、技tag強化、攻撃・防御補正、タイプ無効、HP満タン時効果を対応し、単純なON/OFFで表せる条件付き効果は「条件を満たす」で切り替える。技tagはChampions move metadataから生成する。
- 「ポケモン対戦の基礎」は8カテゴリの共通定義と構造化記事データを使用し、計47記事を公開。全記事にreading order 1〜47を設定し、一覧の番号付きコースとテーマ別カテゴリの両方からアクセスできる。
- 全47記事と8カテゴリはstable icon IDを持ち、一覧・記事見出し・関連記事で共通のインラインSVGアイコンを表示。記事本文のConcept Diagramは表示しない。
...
- カテゴリ別記事数: まず知っておきたいこと5、ダメージ8、素早さ6、状態異常・能力変化9、場の効果7、HPと数値3、パーティーと考え方6、対戦用語3。
...
2. 「ポケモン対戦の基礎」の次の記事群は、既存47記事の利用状況と人間レビューを確認してから優先度を決める。
