...
2026-09-21（battle-basicsへの共通ビジュアルシステム導入後に更新）

## Current focus

初心者向けの学習コンテンツ「ポケモン対戦の基礎」は、導入15記事と第2弾30記事の計45記事を、第1〜45回の連続したコースとして公開しています。全記事と8カテゴリに共通SiteIconを設定し、図解が有効な24記事には軽量なConcept Diagramを導入済みです。
...
- 全45記事と8カテゴリはstable icon IDを持ち、一覧・記事見出し・関連記事で共通のインラインSVGアイコンを表示。24記事は共通の図解枠、HPバー、比較カード、フロー表現によるConcept Diagramを表示する。
...
- 図解は本文を置き換えず、本文だけでも理解できる状態を保つ。数値を含む図は本文と同じChampions仕様を使い、色だけで意味を伝えない。
