...
type Move = { id: string; displayNameJa: string; type: string; damageClass: string; isCoverageMove: boolean };
...
export function PartyChecker({ singles, doubles, singlesAvailable, doublesAvailable, season, updatedAt }: { singles: RankedPokemon[]; doubles: RankedPokemon[]; singlesAvailable: RankedPokemon[]; doublesAvailable: RankedPokemon[]; season: string; updatedAt: string }) {
  ...
  return <>
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="p-4">
        <div className="mb-3 flex items-end justify-between"><div><p className="text-xs font-bold text-blue-700">YOUR PARTY</p><h2 className="text-lg font-black">パーティーを選択</h2></div><span className="text-xs text-slate-400">{party.length} / 6</span></div>
    ...
    <Sheet open={pickerSlot !== null} onOpenChange={(open) => !open && setPickerSlot(null)}><SheetOverlay onClick={() => setPickerSlot(null)} /><SheetContent>
      ...
      <input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="名前で検索" className="mt-4 h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-base outline-none focus:border-blue-500" />
    ...
    <Sheet open={detail !== null} onOpenChange={(open) => !open && setDetail(null)}><SheetOverlay onClick={() => setDetail(null)} /><SheetContent>{detail && <>
      ...
      <div className="mt-5 divide-y divide-slate-100 border-y border-slate-100">{detail.coverage.members.map((member, index) => { const pokemon = party[index]; const groups = [...new Map(member.effectiveMoves.map((move) => [`${move.type}-${move.multiplier}`, { type: move.type, multiplier: move.multiplier, moves: member.effectiveMoves.filter((item) => item.type === move.type && item.multiplier === move.multiplier) }])).values()]; return <div key={member.id} className="flex gap-3 py-3"><img src={pokemon.sprite} alt="" className="size-11 shrink-0 object-contain" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{getPokemonDisplayNameJa(pokemon.id, pokemon.name)}</p>{member.canHitWeakness ? <div className="mt-2 space-y-2">{groups.map((group) => <div key={`${group.type}-${group.multiplier}`}><div className="flex items-center gap-2"><TypeBadge type={group.type} /><span className="text-xs font-black text-blue-700">{group.multiplier}倍</span></div><ul className="mt-1 space-y-0.5 pl-1 text-xs text-slate-600">{group.moves.map((move) => <li key={move.moveId}>・{move.displayNameJa}</li>)}</ul></div>)}</div> : <p className="mt-1 text-xs text-slate-400">有効な技なし</p>}</div><div className={`shrink-0 text-sm font-black ${member.canHitWeakness ? "text-blue-700" : "text-slate-300"}`}>{member.bestMultiplier}×</div></div>; })}</div>
    </>}</SheetContent></Sheet>
  </>;
}
