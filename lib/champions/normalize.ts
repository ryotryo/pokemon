...
export function getUsageRank(summary: any): number | null {
  const rank = Number(summary?.position ?? summary?.rows?.find((row: any) => row.column_position)?.column_position);
...
export function getRanking(indexPokemon: any[], season: string, format: BattleFormat, limit?: number) {
  return indexPokemon
    ...
    .filter((item): item is { entry: any; rank: number } => item.rank !== null)
...
export function normalizePokemon(entry: any, rank: number, battle: any, moves: Record<string, MoveMasterEntry>, namesJa: Record<string, string>): ChampionPokemon {
  ...
  const forms = getAttachedForms(entry).map((form: any) => ({
    ...
    types: form.types, sprite: getChampionsSprite(form.slug, form.image_path),
  }));
