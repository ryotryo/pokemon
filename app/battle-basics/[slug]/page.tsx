...
export default async function Page({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const article=battleBasicsArticleBySlug.get(slug);if(!article)notFound();return <main className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-2xl px-3 py-7 sm:px-6 sm:py-12"><header className="mb-3"><SiteLogo compact/></header><BattleBasicsArticle article={article}/><footer className="mt-9 border-t border-slate-200 pt-5 text-xs leading-5 text-slate-500">画像: <a href="https://championsbattledata.com/" className="underline">Champions Battle Data</a><br/>Pokémon、Nintendo、Game Freak、Creaturesとは関係のない非公式コンテンツです。</footer></div></main>}
