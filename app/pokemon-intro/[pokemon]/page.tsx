...
export default async function Page({params}:{params:Promise<{pokemon:string}>}){const {pokemon:id}=await params;const intro=pokemonIntroById.get(id);const entry=index.pokemon.find(p=>p.id===id);const stats=speed.pokemon.find(p=>p.id===id)?.baseStats;if(!intro||!entry||!stats)notFound();return <main className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-2xl px-3 py-7 sm:px-6 sm:py-12"><header className="mb-3"><SiteLogo compact/></header><PokemonIntroArticle intro={intro} pokemon={entry} stats={stats}/><footer className="mt-9 border-t border-slate-200 pt-5 text-xs leading-5 text-slate-500">Pokémon、Nintendo、Game Freak、Creaturesとは関係のない非公式コンテンツです。</footer></div></main>}
