...
export const battleBasicsArticles: BattleBasicsArticle[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: "まずは何が困るかを知る", paragraphs: ["状態異常ごとに、HP・火力・素早さ・行動への影響が異なります。特に確率や回復までの回数は、Pokémon Championsのルールで確認することが大切です。"], bullets: ["やけど：毎ターンHPが減り、物理技のダメージも下がる", "まひ：素早さが1/2になり、12.5%で技を使えない", "どく：毎ターンHPが減る", "もうどく：経過するほど毎ターンのダメージが増える", "ねむり：1回目は技を使えず、2回目は1/3で起き、3回目は必ず起きる", "こおり：多くの技を使えず、技を使う時に25%で回復し、3回目は必ず回復する"] },
  ...
  extraArticle("paralysis","まひになるとどうなる？","素早さ低下と行動不能の可能性を説明します。","status",[
    ...
    {heading:"12.5%で技を出せない",paragraphs:["技を使おうとするたびに12.5%の確率で体がしびれ、そのターンは選んだ技を出せません。従来作でよく知られる25%ではなく、Pokémon Championsでは12.5%です。"]},
    {heading:"速い相手を抑える",paragraphs:["素早さと行動の両方を邪魔するため、速いアタッカーへの対策になります。でんきタイプのポケモンは基本的にまひ状態になりません。"],takeaway:"まひは素早さが1/2。さらに12.5%で技を使えず、先後関係と計画を崩します。"}
  ...
  extraArticle("sleep","ねむりになるとどうなる？","眠っている間の行動制限と、起きるまでの考え方を説明します。","status",[
    {heading:"多くの技を使えなくなる",paragraphs:["ねむり状態では、いびき・ねごと以外の技を基本的に使えません。起きる判定は、そのポケモンが技を使おうとする時に進みます。"]},
    {heading:"2回目は1/3、3回目は必ず起きる",paragraphs:["ねむり状態になってから1回目は起きられず、技を使えません。2回目は1/3の確率で起き、3回目は必ず起きます。最初に眠れなかった回を含めて数えるのがポイントです。", "技の「ねむる」で眠った場合は、通常のランダムな回復とは異なり、3回目まで回復しません。"]},
    {heading:"交代しても眠りは残る",paragraphs:["控えへ戻しても、ねむり状態は治りません。眠ったポケモンを残すか、別の味方で戦うかを考えます。特性や場の効果で眠りを防げる場合もあります。"],takeaway:"通常のねむりは1回目に行動不可、2回目は1/3で起床、3回目は必ず起床します。"}
  ...
  extraArticle("hp-odd-even","HPは奇数と偶数、何が違う？","割合ダメージの端数が、残りHPへどう影響するかを例で説明します。","numbers",[
    {heading:"基本は、端数を残せるHPが扱いやすい",paragraphs:["最大HPの1/2、1/4、1/8、1/16などを使う多くの効果では、端数が切り捨てられます。たとえばHP200と201は、1/4がどちらも50、1/8がどちらも25です。消費やダメージが同じなら、最初から1多いHP201には最後に1が残ります。", "このため対戦では「HPは奇数が基本」と言われます。ただし、本当に見るべきなのは奇数・偶数だけではなく、4・8・16など効果の分母で割った余りです。"]},
    {heading:"みがわりを4回使える",paragraphs:["みがわりは最大HPの1/4を切り捨てた量を消費し、現在HPが消費量より多いときだけ使えます。HP200では1回50を使うため、3回後は50。4回目は使えません。HP201なら4回使って1残ります。", "同じ考え方で、最大HPが4の倍数でなければ端数が残ります。たとえばHP202も消費は50なので4回使えます。つまり、ここでは「奇数」より正確には「4の倍数を避ける」ことが重要です。"],facts:[{label:"HP200",value:"3回まで・残り50"},{label:"HP201",value:"4回後・残り1"}]},
    {heading:"ステルスロックや定数ダメージを1回多く耐えることがある",paragraphs:["いわ等倍のステルスロックと通常のどくは最大HPの1/8です。HP200も201も1回25ダメージですが、200は8回で0、201は8回受けても1残り、9回目で倒れます。ステルスロックなら、そのぶん場へ出られる回数が1回増える例です。", "やけどや砂嵐は最大HPの1/16です。HP160と161ではどちらも1回10ダメージなので、161なら16回受けた後に1残ります。いのちのたまは攻撃成功ごとに最大HPの1/10を失うため、HP150と151でも同じ関係になります。", "ただし実戦では相手の攻撃や別の削りも受けます。端数を残したからといって、必ず丸々1ターン増えるとは限りません。"],facts:[{label:"1/8の例",value:"HP200→8回で0"},{label:"端数あり",value:"HP201→8回後1"}]},
    {heading:"はらだいこ＋オボンのみは偶数HPが分かりやすい",paragraphs:["はらだいこは最大HPの1/2を減らします。オボンのみは残りHPが最大HPの半分以下になると発動するため、満タンのHP200から使えば100になり、その場で発動します。", "HP201では1/2を切り捨てた100を減らして101残ります。これは半分より多いため、満タンからはらだいこを使っただけではオボンのみが発動しません。こうした「ちょうど半分以下へ入りたい」場面では、偶数HPに明確な利点があります。"],facts:[{label:"HP200",value:"200→100・発動"},{label:"HP201",value:"201→101・未発動"}]},
    {heading:"自分でHPを削り、狙った回数で退場する",paragraphs:["ゴーストタイプが使うのろいは、自分の最大HPの1/2を減らして相手をのろい状態にします。HP200なら100ずつ減るため、回復がなければ2回目で自分も倒れます。相手へ効果を残しながら退場し、控えを安全に出したいときは、この偶数HPが目的に合う場合があります。", "HP201では消費が100になり、2回使っても1残ります。長く場に残りたいなら利点ですが、2回目で退場したい計画とはずれます。はらだいこやのろい以外でも、最大HPの半分を使う効果と発動条件を組み合わせるときは同じ確認が必要です。"]},
    {heading:"奇数か偶数かではなく、狙う効果から決める",paragraphs:["みがわりで端数を残すなら4の倍数を避ける、1/8ダメージを意識するなら8の倍数との関係を見る、たべのこしの回復量を増やすなら16で割った値を見る、というように目的から逆算します。", "たとえば最大HP160のたべのこし回復は10ですが、159では9です。耐える回数だけを見てHPを減らすと、回復量を失う場合もあります。ほかの能力へ回せる数値との兼ね合いも含めて決めましょう。"],takeaway:"端数を残したい場面では奇数が便利。半分以下の発動や予定どおりの退場を狙うなら偶数が有利なこともあります。"}
  ...
  extraArticle("freeze","こおり状態になるとどうなる？","多くの技を使えない状態と、Pokémon Championsで回復する確率・回数を説明します。","status",[
    {heading:"多くの技を使えなくなる",paragraphs:["こおり状態のポケモンが技を選ぶと、まずこおりが解けるか判定されます。解けなければ、そのターンは選んだ技を使えません。やけどやどくのような毎ターンのHPダメージはありませんが、行動できない間に相手へ攻撃や交代の機会を渡します。", "こおりタイプのポケモンは基本的にこおり状態になりません。特性などで防ぐ場合もあります。"]},
    {heading:"1・2回目は25%、3回目は必ず解ける",paragraphs:["Pokémon Championsでは、こおり状態になった後に技を使おうとする1回目と2回目は、それぞれ25%の確率でこおりが解けます。解けた場合は、そのまま選んだ技を使えます。", "2回とも解けなかった場合でも、3回目に技を使おうとしたときは必ず解けます。従来作のように長い間ずっと解けないことはありません。"],facts:[{label:"1回目",value:"25%で回復"},{label:"2回目",value:"25%で回復"},{label:"3回目",value:"必ず回復"}]},
    {heading:"交代では治らず、回数も進まない",paragraphs:["控えへ交代しても、こおり状態は残ります。技を使おうとするまで回復判定は行われないため、交代しているだけでは3回目の確定回復へ近づきません。場に残して回復判定を進めるか、いったん控えへ戻して安全を確保するかを考えます。"]},
    {heading:"一部の炎技などですぐ解けることがある",paragraphs:["一部の炎タイプの技など、こおり状態でも使えて、使うと自分のこおりを解く技があります。また、相手の炎タイプの攻撃を受けたときなどに解ける場合もあります。すべての炎技が同じ扱いではないため、技の説明を確認しましょう。"],takeaway:"Championsのこおりは1・2回目に各25%で回復し、解けなくても3回目の行動時には必ず回復します。"}
  ],["status-conditions","sleep","switching"],["move-search","pokemon-intro"]),
  extraArticle("confusion","こんらん状態になるとどうなる？","技を使えるか自分を攻撃するかの判定と、交代で治る性質を説明します。","status",[
    {heading:"状態異常とは別にかかる一時的な状態",paragraphs:["こんらんは、やけど・まひ・どく・ねむり・こおりとは別の一時的な状態です。そのため、まひしたポケモンが同時にこんらんするなど、主要な状態異常と重なることがあります。", "画面に状態異常の印が1つ付いているかだけで判断せず、こんらんしたという表示も覚えておきましょう。"]},
    {heading:"約1/3で自分を攻撃してしまう",paragraphs:["こんらん中に技を使おうとすると、約1/3の確率で自分を攻撃し、選んだ技を使えません。残りの場合は選んだ技を通常どおり使います。まひなども重なっていると、それぞれの判定で行動を止められることがあります。", "自分への攻撃は威力40相当の物理攻撃として、自分の攻撃と防御を使ってダメージを計算します。技のタイプ、タイプ一致、相性はなく、攻撃が高く防御が低いポケモンほど自傷ダメージが大きくなりやすい仕組みです。"],facts:[{label:"自分を攻撃",value:"約1/3"},{label:"通常どおり行動",value:"約2/3"}]},
    {heading:"数回の行動か、交代で治る",paragraphs:["こんらんは永遠には続かず、技を使おうとする回数が進むと自然に治ります。自然に治るまでの長さは一定ではないため、「次は必ず動ける」とは限りません。", "主要な状態異常と違い、控えへ交代するとこんらんは治ります。大切なポケモンの自傷を避けたいときは交代が確実ですが、交代先が相手の攻撃を受ける点には注意します。"]},
    {heading:"確率だけに任せず、必要なら交代する",paragraphs:["そのターンに動けなくても耐えられるなら、場に残って攻撃する選択があります。一方、自傷ダメージで倒れそう、またはその技を失敗すると負け筋になる場面では、受けられる味方への交代が安全です。特性でこんらんを防ぐポケモンもいます。"],takeaway:"こんらん中は約1/3で自分を攻撃します。主要な状態異常とは重なりますが、交代すれば治ります。"}
  ],["status-conditions","switching","abilities"],["pokemon-intro"]),
];
