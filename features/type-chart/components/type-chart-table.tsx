        ...
        <tbody>
          {TYPE_ORDER.map((attackType) => <tr key={attackType}>
            ...
            {TYPE_ORDER.map((defenseType) => {
              ...
              return <td key={defenseType} aria-label={`${TYPE_NAMES_JA[attackType]}技を${TYPE_NAMES_JA[defenseType]}タイプへ使うと${multiplier}倍`} className={`h-5 border-b border-r border-slate-100 p-0 text-center tabular-nums last:border-r-0 ${cellStyles[multiplier] ?? "text-slate-300"}`}>{formatMultiplier(multiplier)}</td>;
            })}
          </tr>)}
        </tbody>
      </table>
