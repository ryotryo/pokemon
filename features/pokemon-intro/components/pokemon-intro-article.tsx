...
export function PokemonIntroArticle({ intro, pokemon, stats }: { intro: PokemonIntro; pokemon: UsageRankingPokemon; stats: PokemonBaseStats }) {
  return <article>
    ...
    <div className="mt-7 space-y-8">
      ...
      <section><Heading>よく使う技</Heading><div className="mt-3 divide-y divide-slate-100 rounded-2xl border border-slate-200 bg-white px-4">{intro.featuredMoves.map(move=><div key={move.name} className="py-3"><h3 className="text-sm font-black">{move.name}</h3><p className="mt-1 text-xs leading-6 text-slate-600">{move.purpose}</p></div>)}</div></section>
