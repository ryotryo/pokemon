...
import { hasPokemonIntro } from "@/content/pokemon-intros";
import { UsageDetail } from "./usage-detail";

export function UsageDetailPage({ pokemon }: { pokemon: UsagePokemonPageData }) {
  const introId = hasPokemonIntro(pokemon.id) ? pokemon.id : pokemon.megaForms.find((form) => hasPokemonIntro(form.id))?.id;
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <div className="mx-auto max-w-2xl px-3 py-7 sm:px-6 sm:py-12">
        ...
        <Suspense fallback={<div className="h-64 animate-pulse rounded-2xl bg-slate-200" />}>
          <UsageDetail pokemon={pokemon} hasGuide={hasPokemonGuide(pokemon.id)} introId={introId} />
        </Suspense>
