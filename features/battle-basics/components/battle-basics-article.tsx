...
export function BattleBasicsArticle({ article }: { article: BattleBasicsArticle }) {
  ...
  return <article>
    ...
    <header className="mt-1 border-b border-slate-200 pb-5">
      <p className="text-xs font-bold text-blue-700">{article.beginnerCourseOrder ? `完全初心者コース 第${article.beginnerCourseOrder}回` : "ポケモン対戦の基礎"}</p>
      <div className="mt-2 flex items-start gap-3">{article.icon ? <span aria-hidden="true" className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-700"><SiteIcon name={article.icon} className="size-6" /></span> : null}<h1 className="text-2xl font-black tracking-tight sm:text-3xl">{article.title}</h1></div>
    ...
    {article.beginnerCourseOrder ? <nav aria-label="初心者コースの記事移動" className="mt-9 grid grid-cols-2 gap-2 border-t border-slate-200 pt-5">
    ...
    </nav> : <nav aria-label="記事の移動" className="mt-9 border-t border-slate-200 pt-5"><Link href="/battle-basics/" className="text-sm font-bold text-blue-700">← 同じカテゴリの記事を一覧で見る</Link></nav>}

    {related.length ? <section className="mt-8"><h2 className="text-lg font-black">関連する基礎記事</h2><ul className="mt-2 divide-y divide-slate-100 rounded-2xl border border-slate-200 bg-white px-4">{related.map((entry) => <li key={entry.slug}><Link href={`/battle-basics/${entry.slug}/`} className="flex min-h-12 items-center gap-2.5 py-2 text-sm font-bold text-slate-700 hover:text-blue-700">{entry.icon ? <SiteIcon name={entry.icon} className="size-4.5 shrink-0 text-slate-500" /> : null}<span className="min-w-0 flex-1">{entry.title}</span><span aria-hidden="true" className="text-blue-600">→</span></Link></li>)}</ul></section> : null}
