...
export function BattleBasicsIndex() {
  return <>
    <section aria-labelledby="beginner-course" className="rounded-2xl border border-blue-100 bg-white p-4 shadow-sm sm:p-5">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-600">第1回から順番に読むと、対戦画面の見方から試合の考え方まで学べます。気になる記事だけ読んでも大丈夫です。</p>
      ...
      <div className="mt-3 space-y-3">
        {[...battleBasicsCategories].sort((a, b) => a.order - b.order).map((category) => {
          ...
          return <section key={category.id} className="rounded-2xl border border-slate-200 bg-white p-4">
            <div className="flex gap-3"><span className="text-sm font-black text-blue-600">{category.order}</span><div><h3 className="font-black">{category.name}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{category.description}</p></div></div>
            {articles.length > 0 ? <ul className="mt-3 divide-y divide-slate-100 border-t border-slate-100">{articles.map((article) => <li key={article.slug}><Link href={`/battle-basics/${article.slug}/`} className="flex min-h-11 items-center justify-between gap-3 py-2 text-sm font-bold text-slate-700 hover:text-blue-700"><span>{article.title}</span><span aria-hidden="true" className="text-blue-600">→</span></Link></li>)}</ul> : <p className="mt-3 text-xs text-slate-400">公開中の記事はまだありません。</p>}
          </section>;
        })}
      </div>
    </section>
