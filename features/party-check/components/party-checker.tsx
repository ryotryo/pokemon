...
export function PartyChecker({ singles, doubles, singlesAvailable, doublesAvailable, season, seasonLabel, dailyDataPeriod, updatedAt }: { singles: RankedPokemon[]; doubles: RankedPokemon[]; singlesAvailable: RankedPokemon[]; doublesAvailable: RankedPokemon[]; season: string; seasonLabel?: string; dailyDataPeriod?: { start: string; end: string }; updatedAt: string }) {
  ...
  return <>
    ...
    <Sheet open={partyDetail !== null} onOpenChange={(open) => !open && setPartyDetail(null)}><SheetOverlay onClick={() => setPartyDetail(null)} /><SheetContent>{partyDetail && <>
      ...
      <ol className="mt-2 divide-y divide-slate-100 border-y border-slate-100">{[...partyDetail.moves].sort((a, b) => (b.usage ?? Number.NEGATIVE_INFINITY) - (a.usage ?? Number.NEGATIVE_INFINITY) || (a.rank ?? Number.MAX_SAFE_INTEGER) - (b.rank ?? Number.MAX_SAFE_INTEGER)).map((move, index) => <li key={move.id} className="flex min-h-16 items-center gap-3 py-2"><span className="w-5 shrink-0 text-right text-xs font-bold text-slate-400">{index + 1}</span><div className="min-w-0"><p className="text-sm font-bold">{move.displayNameJa}</p><div className="mt-1 flex items-center gap-1 text-[11px] text-slate-500"><span>タイプ：</span><TypeBadge type={move.type} /></div>{typeof move.usage === "number" && Number.isFinite(move.usage) && <p className="mt-1 text-xs text-slate-500">使用率 {move.usage}%</p>}</div></li>)}</ol>
    </>}</SheetContent></Sheet>
